...
var redisServiceURL = devConfig ? devConfig.redisServiceURL : process.env.redisServiceURL;
var inteltsqueryURL = devConfig ? devConfig.inteltsqueryURL : process.env.inteltsqueryURL;

console.log("redisServiceURL: "+redisServiceURL);
...
if (!config.isUaaConfigured()) {
  ...
  app.get('/callback', passport.authenticate('predix', {
    ...
    if (redisServiceURL && redisServiceURL.indexOf('https') === 0) {
      var softFailureEndpoint = redisServiceURL+"/softFailures";
      var hardFailureEndpoint = redisServiceURL+"/hardFailures";
      var sensorStatusEndpoint = redisServiceURL+"/sensorStatus";
      var sensorResetEndpoint = redisServiceURL+"/resetSensor";
      app.post('/queryMicroservice/*', passport.authenticate('main', { noredirect: true}),
      // if calling a secure microservice, you can use this middleware to add a client token.
      proxy.addClientTokenMiddleware,
      proxy.customProxyMiddleware('/queryMicroservice', sensorStatusEndpoint),
      proxy.customProxyMiddleware('/queryMicroservice', softFailureEndpoint),
      proxy.customProxyMiddleware('/queryMicroservice', hardFailureEndpoint),
      proxy.customProxyMiddleware('/queryMicroservice', sensorResetEndpoint));
    }
    //Call microservice to get Redis data
    app.post('/queryMicroservice',
    function(req,res,next){
        proxy.queryMicroservice(req,serviceUrl,proxy.tmpToken, function (data) {
            res.send(data);
          },function(data,err){
            res.send({succes:false,message:"something wrong with the microservice ",err:err})
          });
    });
  // example of calling a custom microservice.
   if (inteltsqueryURL && inteltsqueryURL.indexOf('https') === 0) {
     app.all('/timeSeriesQuery/*', passport.authenticate('main', { noredirect: true}),
        //if calling a secure microservice, you can use this middleware to add a client token.
     proxy.addClientTokenMiddleware,
     proxy.customProxyMiddleware('/timeSeriesQuery', inteltsqueryURL)
     );
   }
...
app.get('/config', function(req, res) {
  ...
  if (config.isAssetConfigured()) {
    title = "Intel Filter Fan Unit Gateway System";
  }
