...
if(node_env === 'development') {
  // use localConfig file
	...
	settings.redisServiceURL = devConfig.redisServiceURL;
	...
	var timeseriesquery = vcapsServices['IntelTSQuery'];
	...
	if(timeseriesquery) {
		settings.inteltsqueryURL = timeseriesquery[0].credentials.uri;
	}
...
settings.buildVcapObjectFromLocalConfig = function(config) {
	...
	if (config.inteltsqueryURL) {
		vcapObj['IntelTSQuery'] = [{
			credentials: {
				uri: config.inteltsqueryURL
			}
		}];
	}
	return vcapObj;
