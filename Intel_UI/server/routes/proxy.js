...
var getClientToken = function(successCallback, errorCallback) {
	...
	request(options, function(err, response, body) {
		if (!err && response.statusCode == 200) {
			...
			//tmpToken = JSON.stringify(clientTokenResponse['access_token']);
			//console.log('access_token: '+tmpToken)
	...
	if (req.session) {
		...
		if (!req.session.clientToken) {
			...
			getClientToken(function(token) {
				...
				tmpToken=req.session.clientToken;
...
var queryMicroservice=function(req,urlConfig,tokenRedis,successCallback,errorCallback){
				var request=require('request');
				//tmpToken = (tokenRedis).split("bearer ").join("")
				console.log("tmptoken::" + tokenRedis)
				var options = {
					method: 'POST',
					url: urlConfig,
					body: req.body,
				...
				request(options, function(err, response, body) {
					if (!err && response.statusCode == 200) {
						...
						errorCallback(body,response);
						console.log('ERROR getTimeSeriesData: ' +response  + ' ' + body);
					}
				});
};
module.exports = {
	...
	queryMicroservice:queryMicroservice
};
